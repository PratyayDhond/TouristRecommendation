...
def get_coordinates(geotags):
    try:
        if 'GPSLatitude' in geotags and 'GPSLongitude' in geotags and 'GPSLatitudeRef' in geotags and 'GPSLongitudeRef' in geotags:
            ...
            lat = get_decimal_from_dms(geotags['GPSLatitude'], 'N')
            lon = get_decimal_from_dms(geotags['GPSLongitude'], 'E')
            return lat, lon
        else:
...
def write_to_csv(filename, data):
    df = pd.DataFrame(data, columns=['Image', 'lat', 'lon', 'timestamp'])
...
if not sys.stdin.isatty():  # If script is called with a pipe
    ...
    if os.path.isdir(subdir_path):
        ...
        for filename in os.listdir(subdir_path):
            if filename.lower().endswith(".jpg") or filename.lower().endswith(".jpeg"):
                ...
                try:
                    geotags, timestamp = extract_geotagging_data(file_path)
                    coordinates = get_coordinates(geotags)

                    if coordinates is not None:
                        data.append([filename, *coordinates, timestamp])
                except ValueError:
...
print('Data converted to coordinates.json successfully.')
